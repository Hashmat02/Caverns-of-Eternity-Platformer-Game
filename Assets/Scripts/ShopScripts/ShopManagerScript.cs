...
public class ShopManagerScript : MonoBehaviour
{
    ...
    public int[,] shopItems = new int[4, 6]; // [property, itemID]
    public TextMeshProUGUI CrystalsTXT;
    private Dictionary<int, int> inventory = new Dictionary<int, int>(); // Dictionary to store inventory
    ...
    public void Checkout()
    {
        ...
        int currentCrystals = CrystalsManager.crystalCount;

        if (currentCrystals >= totalCost)
        {
            CrystalsManager.instance.subtract(totalCost); // Subtract crystals
            UpdateCrystalsText(); // Update crystals display
        ...
        {
            Debug.Log("Not enough crystals for checkout");
        }
    }

    public void SaveData()
    {
        // No need to save crystals separately here, handled by CrystalsManager
        string inventoryJson = JsonConvert.SerializeObject(inventory);
        PlayerPrefs.SetString("Inventory", inventoryJson);
    ...
    {
        CrystalsTXT.text = "Crystals: " + CrystalsManager.crystalCount.ToString(); // Update crystals display
    }
